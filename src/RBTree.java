...
public class RBTree<K extends Comparable<? super K>,V>
{
    ...
    private void deleteCase1(Node<K,V> n) { // 삭제하려는 노드가 루트노드 일때
        if (n.parent == null)
            return;
        else
            deleteCase2(n);
    }
    private void deleteCase2(Node<K,V> n) {  // 삭제하려는 노드의 형제노드가 빨간색인 경우
        if (nodeColor(n.sibling()) == Color.RED) {
            n.parent.color = Color.RED;  // 부모노드를 빨간색으로 변경하고 형제 노드를 검정 색으로 변경한 후 회전
    ...
    private void deleteCase3(Node<K,V> n) { // 삭제하려는 노드의 부모와 형제 그리고 형제의 자식들 모두 검정 노드인 경우
        if (nodeColor(n.parent) == Color.BLACK &&
                nodeColor(n.sibling()) == Color.BLACK &&
                nodeColor(n.sibling().left) == Color.BLACK &&
                nodeColor(n.sibling().right) == Color.BLACK)
        {
            n.sibling().color = Color.RED; // 형제노드를 빨간색으로 변경
            deleteCase1(n.parent);
        }
        else
            deleteCase4(n);
    }
    private void deleteCase4(Node<K,V> n) { // 삭제하려는 노드의 형제와 형제 자식들은 검정이지만 부모가 빨간 노드일 경우
        if (nodeColor(n.parent) == Color.RED &&
                nodeColor(n.sibling()) == Color.BLACK &&
                nodeColor(n.sibling().left) == Color.BLACK &&
                nodeColor(n.sibling().right) == Color.BLACK)
        {
            n.sibling().color = Color.RED; // 형제와 부모의 색을 변경
            n.parent.color = Color.BLACK;
        }
        else
            deleteCase5(n);
    ...
    private void deleteCase5(Node<K,V> n) {
        if (n == n.parent.left && // 삭제 할 노드가 부모의 왼쪽 노드 이고 형제가 블랙노드,
                                    // 형제의 자식이 1개는 레드노드 1개는 블랙 노드 일 경우
                nodeColor(n.sibling()) == Color.BLACK &&
                nodeColor(n.sibling().left) == Color.RED &&
                nodeColor(n.sibling().right) == Color.BLACK)
        {
            n.sibling().color = Color.RED; // 형제노드의 색을 레드로 변경하고 형제 노드의 자식 색을 모두 블랙으로 변경
            n.sibling().left.color = Color.BLACK;
            rotateRight(n.sibling());
        }
        else if (n == n.parent.right && // 삭제 할 노드가 부모의 오른쪽쪽 노드 이고 형제가 블랙노드,
                                          // 형제의 자식이 1개는 레드노드 1개는 블랙 노드 일 경우
                nodeColor(n.sibling()) == Color.BLACK &&
